...
import { Configuration, MikroORM, RequestContext } from '@mikro-orm/core';
import { SqlEntityManager, PostgreSqlDriver } from '@mikro-orm/postgresql';
import { NODE_ENV, PORT, LOG_FORMAT, ORIGIN, CREDENTIALS } from '@config';
import { DI } from '@databases';
import dbOptions from './databases/mikro-orm.config';
import { UserEntity } from '@/apps/user/users.entity';
import { Routes } from '@/api/routes/routes.interface';
...
class Main {
  ...
  private async connectToDatabase() {
    try {
      DI.orm = await MikroORM.init<PostgreSqlDriver>(dbOptions);
      DI.em = <SqlEntityManager>DI.orm.em.fork();
